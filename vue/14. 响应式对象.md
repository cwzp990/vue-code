...
export function defineReactive (
  ...
  if ((!getter || setter) && arguments.length === 2) {  // 只传递两个参数，如walk()
    val = obj[key]
  }
...
(1)
我们之前提到，访问器属性 a 的 getter/setter 通过闭包引用了一个 Dep 实例对象，即属性 a 用来收集依赖的"筐"，除此之外，访问器属性 a 的 setter/getter 还通过闭包引用着 childOb，且 childOb === data.a.**ob**，所以 childOb.dep === data.a.**ob**.dep
